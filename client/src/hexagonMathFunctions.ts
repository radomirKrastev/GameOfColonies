...
export const generateUniqueCornerCoordinates = (
    gridHexesCornersMap: { x: number; y: number }[][]
) => {
...
export const getAllRoadPositions = (
    hexagonCornersPixelCoordinates: {
        x: number;
        y: number;
    }[]
) => {
    const roads: { a: Point; b: Point }[] = [];
    ...
    for (let i = 0; i < hexagonCornersPixelCoordinates.length; i++) {
        ...
        const upperLeft = hexagonCornersPixelCoordinates.find(
            (corner) =>
                Math.abs(corner.x - hexagonCornersPixelCoordinates[i].x - 70) <
                    1 && corner.y === hexagonCornersPixelCoordinates[i].y - 40
        );
