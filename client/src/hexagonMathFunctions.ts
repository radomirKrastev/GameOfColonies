...
export const orientation = (f0: number, f1: number, f2: number, f3: number, b0: number, b1: number, b2: number, b3: number, start_angle: number) => (
  { f0, f1, f2, f3, b0, b1, b2, b3, start_angle });

export const layout = (hexOrientation: Orientation, hexDimensions: Point, gridStartPoint: Point) => (
 ...
 * @returns 
 */
export const hexToPixel = (layout: GameScreenLayout, h: HexCoordinates) => {
 ...
 * @returns 
 */
export const hexCornerOffset = (layout: GameScreenLayout, corner: number) => {
  const orientation = layout.hexOrientation;
  const size = layout.hexDimensions;
 ...
 * @returns
 */
export const polygonCorners = (layout: GameScreenLayout, hex: HexCoordinates) => {
  const corners = [];
...
export const generateUniqueCornerCoordinates = (gridHexesCornersMap: { x: number, y: number }[][]) => {
  return gridHexesCornersMap.reduce((uniqueCorners, hexagonCorners) => {
    hexagonCorners.forEach(corner => {
      ...
      const areCornersEqual = (a: number, b: number, c: number, d: number) => {
        ...
        const cornersDifference = Math.abs(firstCornerAbsoluteValue - secondCornerAbsoluteValue);
